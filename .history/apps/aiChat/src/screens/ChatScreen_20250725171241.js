...
export default function ChatScreen() {
  ...
  // pulsing dots for typing indicator (tiny animation)
  ...
  const sendChatMessage = async (overrideText, isRegenerate = false) => {
    ...
    try {
      ...
      // 1) call your backend (replace with stream endpoint if you have it)
  ...
  const onLongPressMessage = (msg) => {
    const opts = [
      { text: 'Copy', onPress: () => onCopyMessage(msg) },
      { text: 'Cancel', style: 'cancel' },
    ];
    ...
    Alert.alert('Message actions', '', opts);
  };
    ...
    return (
      ...
      >
        ...
        <View style={styles.messageHeader}>
          ...
          {isAssistant && (
            <TouchableOpacity onPress={() => onCopyMessage(item)}>
        ...
        {isLastMessage && isStreaming && isAssistant ? (
          <Markdown
            style={markdownStyles(theme, textColor)}
          >
            {streamedText || ' '}
          </Markdown>
        ) : (
  ...
  return (
    ...
    >
      ...
      >
        {/* Header (model tag & clear button) */}
        <View style={[styles.header, { borderBottomColor: theme.colors.border }]}>
          <Typography
            variant="caption"
            style={{ color: theme.colors.textSecondary }}
          >
            Model: gpt-4o-mini (fake tag — replace with your model)
          </Typography>
        ...
        {/* Streaming tools: stop / regenerate */}
        {(isStreaming || loading) && (
...
const markdownStyles = (theme, textColor) => ({
  body: {
    color: textColor,
    fontSize: 15,
    lineHeight: 22,
