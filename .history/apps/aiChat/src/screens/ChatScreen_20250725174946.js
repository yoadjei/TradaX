...
export default function ChatScreen() {
  ...
  // Animated values for new messages
    ...
    return (
      <Animated.View
        style={[
          ...
          { opacity: animatedValues[item.id], transform: [{ scale: animatedValues[item.id].interpolate({ inputRange: [0, 1], outputRange: [0.95, 1] }) }] },
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      >
        ...
        <Card style={[styles.disclaimerCard, { backgroundColor: theme.colors.warning + '20' }]}>
          <Typography
            variant="caption"
            style={{ color: theme.colors.text, textAlign: 'center', fontSize: 12 }}
          >
            ⚠️ This AI assistant is for educational purposes only — not financial advice.
          </Typography>
        </Card>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  disclaimerCard: { margin: 16, marginBottom: 8, padding: 10, borderRadius: 8 },
  messagesContainer: { paddingHorizontal: 16, paddingBottom: 16, paddingTop: 8 },
  messageContainer: { marginVertical: 4 },
  userMessageContainer: { alignItems: 'flex-end' },
  aiMessageContainer: { alignItems: 'flex-start' },
  messageCard: { padding: 12, maxWidth: '90%', borderRadius: 12 },
  typingIndicator: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingLeft: 16,
