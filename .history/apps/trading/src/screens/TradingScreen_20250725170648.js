...
import { View, StyleSheet, ScrollView, Modal, Image, Dimensions } from 'react-native';
...
const RANGES = [
  ...
  { key: '6M', days: 180 },
  { key: '1Y', days: 365 },
  { key: 'MAX', days: 1825 }, // 5y for example
];
...
export default function TradingScreen() {
  ...
  const canAfford = useMemo(() => {
    if (tradeType === 'buy') return balance.usd >= totalWithFee;
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Card style={[styles.chartCard, { backgroundColor: theme.colors.surface }]}>
          {/* Removed the "Candlestick chart" / "Line chart" title completely */}

          {/* Chart type toggle (still visible, but you can remove if you want) */}
          <ScrollView
            horizontal
              ...
              <Button
                ...
                style={[
                  styles.modalButton,
                  {
                    ...
                    opacity: (!canAfford || numericAmount <= 0 || (orderType === 'limit' && numericLimitPrice <= 0)) ? 0.6 : 1
                  },
                ]}
              />
            </View>
...
const styles = StyleSheet.create({
  ...
  chartTypeButton: { marginRight: 6 },
  rangeToggle: { flexDirection: 'row', marginBottom: 8 },
  rangeButton: { marginRight: 6 },
