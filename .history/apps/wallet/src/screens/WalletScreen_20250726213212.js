...
function CoinImage({ uri, style, placeholderColor }) {
  ...
  return (
    <View style={style}>
      <Input // dummy invisible input to avoid importing Image from RN; replace with Image if available in @tradax/ui
        style={{ display: 'none' }}
      />
    </View>
  );
      ...
      try {
        ...
        if (Array.isArray(res)) {
          res.forEach(p => {
            ...
            const price =
              Number(p.price ?? p.last ?? p.current_price ?? p.usd ?? 0);
          ...
          Object.keys(res).forEach(k => {
            ...
            const v = res[k];
            const price =
              Number(v?.price ?? v?.last ?? v?.current_price ?? v?.usd ?? 0);
