  ...
  const balances = useMemo(() => {
    return availableAssets.map(a => {
      ...
      return {
        ...
        usd: Number(srv.balance ?? 0) * Number(srv.price ?? 0),
      };
    });
  }, [availableAssets, bySymbol]);
    ...
    <View>
      ...
      <Card style={[styles.quickActions, { backgroundColor: theme.colors.surface }]}>
        ...
        <View style={styles.quickButtons}>
          <Button title="Deposit USD" onPress={() => openModal('deposit', { symbol: 'USD', balance: 0 })} style={styles.quickBtn} />
          <Button title="Withdraw USD" variant="outline" onPress={() => openModal('withdraw', { symbol: 'USD', balance: 0 })} style={styles.quickBtn} />
        </View>
      </Card>
