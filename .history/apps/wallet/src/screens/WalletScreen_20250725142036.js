...
export default function WalletScreen() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      await fetchBalances(false, isMounted);
    };
  ...
  const fetchBalances = async (isRefresh = false, isMountedRef = { value: true }) => {
    // handle both boolean or boolean-like ref passed from effect
    const isMounted = typeof isMountedRef === 'boolean' ? isMountedRef : isMountedRef;
  ...
  const onRefresh = () => fetchBalances(true, true);
  ...
  const renderItem = ({ item }) => {
    ...
    const img = coinImages[symbol] || coinImages[symbol?.toUpperCase()] || undefined;
    ...
    return (
      <Card style={[styles.assetCard, { backgroundColor: theme.colors.surface }]}>
        <View style={styles.assetRow}>
          ...
          {img ? (
            <Image source={{ uri: img }} style={styles.coinImage} />
          ) : (
      ...
      <FlatList
        ListHeaderComponent={
          <View>
            ...
            <Typography variant="h2" style={{ color: theme.colors.text, padding: 20 }}>
              Your Assets
            </Typography>
          </View>
        }
        data={balances}
        keyExtractor={(item, idx) => item.symbol || item.asset || String(item.id) || String(idx)}
