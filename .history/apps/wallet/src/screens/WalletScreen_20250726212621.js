...
export default function WalletScreen() {
  ...
  useEffect(() => {
    ...
    const loadPrices = async () => {
      try {
        const res = await cryptoApi.getPrices?.(); // expected to return array or object
        const map = {};
  ...
  const baseBalances = useMemo(() => {
    return availableAssets.map(a => {
      ...
      const livePrice =
        Number(priceMap[sym]?.price ??
          priceMap[sym]?.last ??
          priceMap[sym]?.current_price ??
          priceMap[sym]?.usd ??
          srv.price ??
          0);
  ...
  }, [availableAssets, bySymbol, totalValue, priceMap]);
  ...
  const balances = useMemo(() => {
    ...
    const filtered = showSearchBar && search
      ? list.filter(
          i =>
              ...
              renderItem={({ item }) => (
                <Pressable
                  onPress={() => {
                    ...
                    setSearch('');
                    // maybe open asset details later
                  }}
                >
                  <View style={[styles.searchRow, { borderBottomColor: theme.colors.border }]}>
                    <View style={styles.assetInfoRow}>
                      <Image source={{ uri: item.image }} style={styles.coinImage} />
                      <View>
                        <Typography variant="body1" style={{ color: theme.colors.text }}>{item.symbol}</Typography>
                        <Typography variant="body2" style={{ color: theme.colors.textSecondary }}>{item.name}</Typography>
                      </View>
                    </View>
                    <View style={{ alignItems: 'flex-end' }}>
                      <Typography variant="body1" style={{ color: theme.colors.text }}>{formatQty(item.balance)}</Typography>
                      <Typography variant="body2" style={{ color: theme.colors.textSecondary }}>{formatPrice(item.usd)}</Typography>
                    </View>
                  </View>
                </Pressable>
              )}
