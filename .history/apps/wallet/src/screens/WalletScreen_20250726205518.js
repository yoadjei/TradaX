// apps/wallet/src/screens/WalletScreen.js
...
import { useWallet } from '@wallet/src/stores/WalletContext';
...
export default function WalletScreen() {
  ...
  const imagesBySymbol = useMemo(() => {
    const m = {};
    AVAILABLE_ASSETS.forEach(a => (m[a.symbol.toUpperCase()] = a.image));
    return m;
  }, []);
  ...
  const renderItem = ({ item }) => {
    ...
    const canWithdraw = Number(item.balance) > 0;
    const canSell = Number(item.balance) > 0;
    const image = imagesBySymbol[symbol];
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <FlatList
        ...
        data={balances}
        keyExtractor={(item, idx) => item.symbol || item.asset || String(idx)}
