...
export default function NewsListScreen() {
  ...
  const fetchNews = async (isRefresh = false) => {
    ...
    try {
      // Ask for more than 10; your newsApi auto-paginates + dedupes
      const articles = await newsApi.getLatestNews({ max: 60 });

      // Just-in-case client-side de-dupe
      const seen = new Set();
      const prepared = [];
      ...
      // Latest first (should already be sorted server-side)
      prepared.sort((a, b) => new Date(b.publishedAt) - new Date(a.publishedAt));
  ...
  // If your backend already sets category & score, keep them.
  // If not, we derive them here:
  const enriched = useMemo(() => {
    return news.map((n, i) => ({
      ...n,
      category: n.category || detectCategory(`${n.title} ${n.description}`),
      score: typeof n.score === 'number' ? n.score : 60 - i,
    }));
  }, [news]);
