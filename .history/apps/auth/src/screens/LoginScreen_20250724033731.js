...
export default function LoginScreen() {
  ...
  const navigation = useNavigation(); // ✅ Use this one only
  const { login: contextLogin } = useAuth(); // Optional: use auth context
    ...
    try {
      ...
      if (response?.token) {
        ...
        await contextLogin({
          ...
          user: response.user || {}, // Adjust based on API structure
        });
