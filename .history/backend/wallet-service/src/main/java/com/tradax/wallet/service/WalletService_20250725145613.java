...
public class WalletService {
    ...
    // ----------------------------------------------------------------
    // DEMO helpers (so you can show balances without authentication)
    // ----------------------------------------------------------------

    @Transactional(readOnly = true)
    public Map<String, Object> getDemoBalancesResponse() {
        List<Wallet> wallets = new ArrayList<>();
        wallets.add(buildDemoWallet("BTC", "Bitcoin", new BigDecimal("0.123456"), new BigDecimal("45000")));
        wallets.add(buildDemoWallet("ETH", "Ethereum", new BigDecimal("2.50"), new BigDecimal("3000")));
        wallets.add(buildDemoWallet("USD", "US Dollar", new BigDecimal("10000"), new BigDecimal("1")));

        BigDecimal total = wallets.stream()
                .map(w -> w.getBalance().multiply(w.getPrice()))
    ...
    private Wallet buildDemoWallet(String symbol, String name, BigDecimal balance, BigDecimal price) {
        Wallet w = new Wallet();
        w.setUserEmail("demo@tradax.local");
        w.setAsset(symbol);
        w.setSymbol(symbol);
        w.setName(name);
    ...
    public Map<String, Object> getPortfolioPerformance(String userEmail) {
        ...
        performance.put("dayChange", new BigDecimal("0.00")); // Placeholder
